...
public class ShabatElev2Algo implements ElevatorAlgo {
    ...
    public int allocateAnElevator(CallForElevator c) {
        ...
        if (_building.numberOfElevetors() > 1) {
            if (c.getSrc() < c.getDest()) { // UP  מספר המקום הנוסע הולך אליו יותר גדול ממספר המקום הנוכחי לכן מכאן מבין שהוא הולך למעלה
                while (ans % 2 == 0) {
                    ans = rand(0, _building.numberOfElevetors()); // this is totally inefficient
                }
            }
            else {  // מספר המקום הנוסע הולך אליו יותר קטן ממספר המקום הנוכחי לכן מכאן מבין שהוא הולך למטה
                ans = 1;
                while (ans % 2 == 1) {
                    ans = rand(0, _building.numberOfElevetors());  // this is totally inefficient
                }
    ...
    private void f0() {
        ...
        if(curr.getState() == Elevator.LEVEL) {
            int dir = this.getDirection();  // -1 down  1 up
            int pos = curr.getPos();  // the number of the floor where is the ele right now
            boolean up2down = false;
            if(dir == UP) {
